...
class _PaginaAvaliacaoState extends State<PaginaAvaliacao> {
  ...
  // Controllers para dados do usuário (opcionais)
  final TextEditingController _nomeController = TextEditingController();
  final TextEditingController _emailController = TextEditingController();
  final TextEditingController _commentsController = TextEditingController();
  ...
  Future<void> enviarAvaliacaoParaFirestore() async {
    ...
    if (todasVazias) {
      return showDialog(
        ...
        builder: (context) => AlertDialog(
          title: const Text('Atenção'),
          content: const Text('Preencha pelo menos uma avaliação para enviar.'),
          actions: [
            TextButton(
              onPressed: () => Navigator.pop(context),
              child: const Text('OK'),
    ...
    try {
      ...
      String usuarioId =
          _nomeController.text.isNotEmpty ? _nomeController.text : 'USR_000123';

      // Construir mapa de avaliações com números de 1 a 5
      final Map<String, dynamic> avaliacoesMap = {};
      avaliacoes.forEach((chave, avaliacao) {
        avaliacoesMap[chave] = avaliacao?.numero ?? 0;
      });
  ...
  Future<void> _mostrarFeedback(BuildContext context) async {
    ...
    // ✅ SEM AWAIT - Deixa o diálogo aparecer sem bloquear
    showDialog(
      context: context,
      barrierDismissible: false, // Não permite fechar clicando fora
      builder: (BuildContext context) {
        return AlertDialog(
          title: const Text('Obrigado!', textAlign: TextAlign.center),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
          ...
          // ✅ SEM ACTIONS - Não há botão para clicar
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(20),
          ),
        );
          ...
          child: Column(
            ...
            children: [
              ...
              if (isLandscape && !isMobile)
                ...
                _buildAvaliacoesGridPortrait(),

              const SizedBox(height: 32),

              // Seção de comentários opcionais
              Text(
              ...
              TextField(
                ...
                maxLines: 4,
                decoration: InputDecoration(
                  hintText: 'Deixe seus comentários aqui...',
                  border: OutlineInputBorder(
  ...
  /// Grid de avaliações em portrait
  Widget _buildAvaliacoesGridPortrait() {
    return Column(
      children: avaliacoes.entries.map((entry) {
        return Column(
          ...
          children: [
            Text(
              nomesExibicao[entry.key] ?? entry.key,
              style: const TextStyle(
                fontSize: 16,
            ...
            EmojiSelector(
              ...
              onChanged: (avaliacao) {
                setState(() {
                  avaliacoes[entry.key] = avaliacao;
                });
              },
            ),
            const SizedBox(height: 24),
  ...
  /// Grid de avaliações em landscape (2 colunas)
  Widget _buildAvaliacoesGridLandscape() {
    final entries = avaliacoes.entries.toList();
    final chunks = <List<MapEntry<String, Avaliacao?>>>[];

    for (int i = 0; i < entries.length; i += 2) {
      chunks.add(entries.sublist(
        i,
        i + 2 > entries.length ? entries.length : i + 2,
      ));
    }
  ...
  /// Dados do usuário em portrait
  Widget _buildDadosGridPortrait() {
    return Column(
      children: [
        TextField(
          controller: _nomeController,
  ...
  /// Dados do usuário em landscape
  Widget _buildDadosGridLandscape() {
    return Row(
      children: [
        Expanded(
          child: TextField(
